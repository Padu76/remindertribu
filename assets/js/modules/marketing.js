  ...
  function showTemplateEditor(template = null, onSave) {
    ...
    modal.querySelector('#save-btn').onclick = () => {
      ...
      const updatedTemplate = {
        id: template?.id || name.toLowerCase().replace(/\s+/g, '_'),
        name,
        category,
        body,
  ...
  async function saveTemplate(template) {
    ...
    try {
      if (typeof storage.saveTemplate === 'function') {
        await storage.saveTemplate(template.id, template);
      } else if (storage.firebase?.db) {
        await storage.firebase.db.collection('templates').doc(template.id).set(template, { merge: true });
      }
